  ...
  const features = [
    {
      ...
      description: "Unleash powerful image intelligence. ImageSorcery empowers your AI agents to accurately identify and locate objects within images using cutting-edge models like YOLO from Ultralytics. With the detect and find tools, your AI can gain precise visual understanding. Crucially, these capabilities allow your agent to autonomously apply computer vision when it's a necessary step to fulfill your broader requests, even if not explicitly commanded.",
    ...
    },
    {
      ...
      description: "Gain granular control over your visuals. ImageSorcery empowers your AI to effortlessly transform images to exact specifications. Tools like rotate allow precise angular adjustments, while crop enables perfect framing for any platform. Additionally, resize offers flexible scaling. Crucially, the get_metainfo tool provides your agent with essential image dimensions and properties, ensuring intelligent and accurate application of these transformations.",
    ...
    },
    {
      ...
      description: "Effortlessly enhance your images by overlaying other visuals. Whether it's adding logos, watermarks, or combining elements, ImageSorcery allows your AI to seamlessly blend images with transparency handling using the overlay tool, creating polished, professional results.",
      demo: "Logo overlay with transparency handling on main image"
    },
    {
      icon: <Type className="w-8 h-8" />,
      title: "Dynamic Text Annotation",
      description: "Add contextual information or creative flair directly onto your images. ImageSorcery empowers your AI to draw custom text at any position, with control over font, size, and color, using the draw_texts tool, perfect for labels, captions, or dynamic overlays.",
      demo: "Adding custom text with different fonts, sizes, and colors to images"
    },
    {
      icon: <Target className="w-8 h-8" />,
      title: "Visual Highlighting & Diagramming",
      description: "Guide attention and illustrate concepts with ease. ImageSorcery allows your AI to draw various shapes directly onto images, including arrows (draw_arrows), circles (draw_circles), and rectangles (draw_rectangles), enabling clear visual communication and annotation for presentations or analysis.",
      demo: "Adding arrows, circles, and rectangles to highlight specific areas"
    },
    {
      icon: <FileText className="w-8 h-8" />,
      title: "Intelligent Text Extraction (OCR)",
      description: "Transform static images into actionable data. ImageSorcery's advanced OCR capabilities enable your AI to accurately extract text from documents, forms, or even specific fields like a name from an ID card, using the ocr tool, turning visual information into editable and searchable data.",
    ...
    },
    {
      ...
      description: "Unlike cloud-based solutions, ImageSorcery processes all your images directly on your local machine. This ensures your sensitive visual data never leaves your environment, providing unparalleled security and privacy. Whether you're interacting with external LLMs like Claude, Gemini, or ChatGPT, or leveraging the absolute security of local models such as Llama or Mistral, your visual data remains entirely within your control.",
      demo: "Data stays inside your computer with shield/lock protection"
    },
    {
      icon: <Puzzle className="w-8 h-8" />,
      title: "Expandable Model Ecosystem",
      description: "For advanced users, ImageSorcery offers unparalleled flexibility by allowing the integration of custom models. Easily download and utilize specialized models from platforms like Hugging Face to extend your AI's capabilities for niche tasks.",
    ...
    <div className="min-h-screen bg-darker-bg">
      ...
      <section className="relative min-h-screen hero-bg flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute top-40 right-20 w-24 h-24 bg-gradient-primary rounded-full opacity-15 animate-float" style={{ animationDelay: '2s' }}></div>
          <div className="absolute bottom-40 left-1/4 w-20 h-20 bg-gradient-primary rounded-full opacity-10 animate-float" style={{ animationDelay: '4s' }}></div>
        </div>
      ...
      <section id="getting-started" className="py-20 bg-gradient-glow">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
...
{agenticSetupCode}
                      ...
                      <Button
                        ...
                        onClick={() => handleCopy(agenticSetupCode, 'agentic')}
                      >
                        {copiedStates.agentic ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                        {copiedStates.agentic ? 'Copied!' : 'Copy Code'}
                      </Button>
                    </div>
...
imagesorcery-mcp --post-install`}
                            ...
                            <Button
                              ...
                              variant="outline"
                              className="absolute top-2 right-2"
                              onClick={() => handleCopy(`python -m venv imagesorcery-mcp\nsource imagesorcery-mcp/bin/activate\npip install imagesorcery-mcp\nimagesorcery-mcp --post-install`, 'linux-install')}
                            >
                              {copiedStates['linux-install'] ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                            </Button>
                          </div>
    ...
    "imagesorcery-mcp": {
      ...
      "timeout": 100
                            ...
                            <Button
                              ...
                              onClick={() => handleCopy(`{\n  "mcpServers": {\n    "imagesorcery-mcp": {\n      "command": "/full/path/to/venv/bin/imagesorcery-mcp",\n      "transportType": "stdio",\n      "timeout": 100\n    }\n  }\n}`, 'linux-config')}
                            >
                              {copiedStates['linux-config'] ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                            </Button>
                          </div>
                        </div>
                      </TabsContent>
...
imagesorcery-mcp --post-install`}
                            ...
                            <Button
                              ...
                              variant="outline"
                              className="absolute top-2 right-2"
                              onClick={() => handleCopy(`python -m venv imagesorcery-mcp\nimagesorcery-mcp\\Scripts\\activate\npip install imagesorcery-mcp\nimagesorcery-mcp --post-install`, 'windows-install')}
                            >
                              {copiedStates['windows-install'] ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                            </Button>
                          </div>
                        </div>
    ...
    "imagesorcery-mcp": {
      ...
      "timeout": 100
                            ...
                            <Button
                              ...
                              onClick={() => handleCopy(`{\n  "mcpServers": {\n    "imagesorcery-mcp": {\n      "command": "C:\\\\full\\\\path\\\\to\\\\venv\\\\Scripts\\\\imagesorcery-mcp.exe",\n      "transportType": "stdio",\n      "timeout": 100\n    }\n  }\n}`, 'windows-config')}
                            >
                              {copiedStates['windows-config'] ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                            </Button>
                          </div>
                        </div>
                      </TabsContent>
      ...
      <section className="py-20">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
              ImageSorcery MCP is trusted and listed across prominent MCP directories and AI client platforms. Click on any platform to explore our listing.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {platforms.map((platform, index) => (
              <a
                ...
                className="glass-card p-6 rounded-xl hover-glow transition-all duration-300 hover:scale-105 text-center"
              >
      ...
      <footer className="py-12 border-t border-border">
        <div className="container mx-auto px-6">
          <div className="text-center">
            <div className="mb-6">
              <h3 className="text-2xl font-bold text-gradient mb-2">ImageSorcery MCP</h3>
              <p className="text-muted-foreground">Empowering AI with Visual Intelligence</p>
            </div>
